...
#endif
  U16				bank = 0;			// default, point to groug #0 -- channels 0-15.
  unsigned long		start_time;
  unsigned long		time_spent;
  float				switchingRate;
...
//  long int			loop;
  ...
  for (;;)
  {
	  ...
	  if ( chan>3 ) 
	  {
		  printf ("%d Enter DAC channel # and ramping outputs [0-3 1-%li, q] : ",chan, max );
		  continue;
	  }
	  if ( times<1 || times>max ) 
	  {
	  ...
	  printf ("DAC channel #%i ramping (-10V_/+10V) ..... ", chan );	fflush(stdout);
